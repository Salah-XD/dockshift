...
**Main-process core — ✅ DONE + verified headlessly under Electron 42 (commits on `feat/voice-dictation-spec`):**
1. ✅ **module DONE** `electron-stt-models.js` — catalog + download(resume)/SHA-256-verify/extract, `listModels`/`getStatus`/`isInstalled`/`modelSupportsLanguage`/`removeModel`. Parakeet v3 SHA-256 pinned; un-pinned models gated. Verified 16/16 (`verify-catalog.mjs`, incl. real network download). *Pending: the `stt:models:*` IPC handlers in electron-main.js (renderer-wiring step below).*
...
4. ✅ **DONE** `stt:models:*` IPC handlers in electron-main.js (list/status/download/remove) + `stt:model:progress` push; `transcription:transcribe` accepts `{ pcm: true, sampleRate }`; `useMicPcm.js` (Web Audio → mono Float32, base64); `VoicePanel.jsx` localNative path (model-download gate + record → PCM → transcribe); engine `unloadAll()` on quit. preload allowlist updated (three-place rule; IPC-sync hook clean). Verified: `verify-app-e2e.cjs` **6/6** (real IPC + **fake-mic 48 kHz → correct transcript**) and `verify-app-ui.cjs` **4/4** (VoicePanel mounts + renders the record control).
   - ✅ **DONE — default flipped:** `DEFAULT_TRANSCRIPTION_PROVIDER_ID = local-parakeet` for every install. Verified (`verify-welcome.cjs`: a profile with no `sttProvider` resolves `activeId = 'local-parakeet'`).
5. ✅ **DONE — welcome "Voice engine" step + auto-download + model-switching.**
   - Welcome screen (`WelcomeWindow.jsx`, new step) lists all catalog models as comparison cards with **accuracy + speed bars + multilingual badge** (driven by `accuracy`/`speed`/`multilingual` now on `getStatus`). Parakeet v3 is the recommended default; **Parakeet v2 (English, top accuracy) is also installable** (checksum pinned + verified); Whisper small/medium show stats + "Soon" (asset names corrected, checksums still to pin + whisper engine path to verify).
   - **Auto-download for every user:** `welcome:complete` persists the chosen provider/model and kicks off the download in the background (broadcast on `stt:model:progress`); the Voice panel also auto-starts the download on open if the model is missing (deduped by an in-flight guard + `getStatus().downloading`).
   - **Model-switching:** `settings.sttModel` (+ validator) resolved in the transcribe handler and threaded as a `modelId` override into the provider, with a same-engine-family guard. Verified `verify-modelswitch.cjs` **4/4** (v3↔v2 override + cross-family fallback) and `verify-welcome.cjs` **10/10** (cards render, default flip, completion persists `sttProvider`/`sttModel`).
6. ✅ **DONE** preload allowlist + IPC handlers in sync (IPC-sync hook clean); `sttModel` settings validator added.
7. ⏳ **REMAINING:** `/verify-build` — package via electron-builder and confirm `asarUnpack` ships the sherpa native addon in the `.exe` (manual gate per AGENTS.md; the e2e ran against the Vite dev renderer + unpacked Electron, not the packaged build). Follow-ups: pin Whisper checksums + verify the whisper engine path to make Whisper installable.
